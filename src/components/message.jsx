...
import knitLogo from '@assets/knitLogo.png'
...
export default function Message() {
  ...
  return (
    <section id="message" className="flex flex-col md:px-32 px-8 py-24 bg-bl-primary text-white">
      <motion.div
          ...
          <div className=" relative justify-center items-center sm:w-[60%]  ">
            <div className="flex bg-bl-secondary    rounded-md shadow-xl">
              {messages.map((message, index) => (
                <div
                  key={message.id}
                  className={`flex flex-col shadow-lg justify-start items-start py-7 gap-10  sm:px-10 px-3  rounded-xl ${slideNo === index + 1 ? '' : 'hidden'
                    }`}
                >
                  <div
        ...
        <div className=" flex  gap-5 py-5 ">
          {messages.map((mess, i) => (
            <div
              ...
              className={`w-3  cursor-pointer h-3 rounded-full ${slideNo == i + 1 ? ' bg-bl-primary ' : 'bg-[#dddce0c5]'
                } `}
            />
          ))}
        </div>
